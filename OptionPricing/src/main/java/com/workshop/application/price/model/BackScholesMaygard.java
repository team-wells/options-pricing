...
public final class BackScholesMaygard {

	public BackScholesMaygard() {
	}

	/**
	 * For carryrate=rate. The black Scholes basic model. For carryrate a zero
	 * value. Black 1976 futures For carryrate != rate Gives cont yield model
	 **/
	public BackScholesMaygard(double carryrate) {
	...
	public void bscholEprice(double sprice, double strike, double volatility, double time, double rate) {
		...
		setcalle(((sprice * Math.exp((brate - rate) * time)) * probd1) - ((strike * Math.exp(-rate * time)) * probd2));
		setpute(((strike * Math.exp(-rate * time)) * p.ncDisfnc(-d2))
				- (sprice * Math.exp((brate - rate) * time)) * p.ncDisfnc(-d1));
		deltac = (Math.exp((brate - rate) * time) * probd1);
		deltap = (Math.exp((brate - rate) * time) * (probd1 - 1));
		gamma = ((densityfunc * (Math.exp((brate - rate) * time))) / (sprice * volatility * Math.sqrt(time)));
		vega = ((sprice * Math.exp((brate - rate) * time)) * densityfunc * Math.sqrt(time));
		...
		thetac = (((-(sprice * Math.exp((brate - rate) * time)) * densityfunc * volatility) / (2 * Math.sqrt(time)))
				- thetaterm1 - thetaterm2);
	}

	private void dvalues(double sprice, double strike, double volatility, double time, double rate) {
		brate = crate == 0.0 ? 0.0 : (brate = crate != rate ? (rate - crate) : rate);
		d1 = ((Math.log(sprice / strike) + (brate + (volatility * volatility) * 0.5) * time)
				/ (volatility * Math.sqrt(time)));
		d2 = (d1 - (volatility * Math.sqrt(time)));
	}

	public void setDelta(double sprice, double strike, double volatility, double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		double probd1 = 0.0;
		double probd2 = 0.0;
	...
	public void setGamma(double sprice, double strike, double volatility, double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		double probd1 = 0.0;
		probd1 = p.ncDisfnc(d1);
	...
	public void setVega(double sprice, double strike, double volatility, double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		double probd1 = 0.0;
		probd1 = p.ncDisfnc(d1);
	...
	public void setTheta(double sprice, double strike, double volatility, double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		...
		double thetaterm3 = ((-(sprice * Math.exp((brate - rate) * time)) * densityfunc * volatility)
				/ (2 * Math.sqrt(time)));
		...
		double thetatermc = ((-(sprice * Math.exp((brate - rate) * time)) * densityfunc * volatility)
				/ (2 * Math.sqrt(time)));
		thetap = (thetatermc + (thetaterma) + (thetatermb));
	}

	public void setRho(double sprice, double strike, double volatility, double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		if (brate != 0.0) {
	...
	public void setElstic(double sprice, double strike, double volatility, double time, double rate) {
		bscholEprice(sprice, strike, volatility, time, rate);
	...
	public void setCarry(double sprice, double strike, double volatility, double time, double rate) {
		Probnormal p = new Probnormal();
		dvalues(sprice, strike, volatility, time, rate);
		carryc = (time * (sprice * Math.exp((brate - rate) * time)) * p.ncDisfnc(d1));
		carryp = (-time * (sprice * Math.exp((brate - rate) * time)) * p.ncDisfnc(-d1));
	}
